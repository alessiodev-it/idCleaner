...
class BaseFolderCleaner(Thread, ABC):
    ...
    def run(self):
        while not self.stopper.is_set():
            try:
                with MailBox(host=self.imap_data["host"], port=self.imap_data["port"]).login(
                    username=self.imap_data["username"], password=self.imap_data["password"]
                ) as mailbox:
    ...
    def _first_time_cleaning(self, mailbox):
        ...
        if (os.getenv("FIRST_TIME") or "").lower() != "true":
            return
        if self.folder_name == FOLDER_NAMES["bin"]:
            return
